 ...
 * RapidAddon
 *
 * @package     RealEstate Sync RapidAddon
...
if (!class_exists('RapidAddon')) {
	...
	class RealEstate_Sync_AddOn_RapidAddon {
		...
		*/
		function run($conditions = array()) {
			...
			add_filter('realestate_sync_addons', array($this, 'wpai_api_register'));
			...
			add_filter('realestate_sync_options_options', array($this, 'wpai_api_options'));
            add_filter('wp_all_import_image_sections', array($this, 'additional_sections'), 10, 1);
            add_filter('realestate_sync_custom_types', array($this, 'filter_post_types'), 10, 2);
            add_filter('realestate_sync_post_list_order', array($this,'sort_post_types'), 10, 1);
            add_filter('wp_all_import_post_type_image', array($this, 'post_type_image'), 10, 1 );
			add_action('realestate_sync_extend_options_featured',  array($this, 'wpai_api_metabox'), 10, 2);
    ...
    <h3 class="hndle" style="margin-top:0;"><span>'.$acf_group['title'].'</span></h3>
		...
		/**
		* 
		* Add an option to RealEstate Sync options list
		*
	...
	//		echo "</pre>";
					...
					foreach ($this->fields as $field_slug => $field_params) {
						...
						switch ($field_params['type']) {
							...
							case 'image':
								...
								$uploaded_image = realestate_sync_API::upload_image($post_id, $image_url_or_path, $download, $importData['logger'], true, "", "images", true, $importData['articleData']);
							...
							case 'file':
								...
								$uploaded_file = realestate_sync_API::upload_image($post_id, $image_url_or_path, $download, $importData['logger'], true, "", "files", true, $importData['articleData']);
		...
		function wpai_api_metabox($post_type, $current_values) {
			...
			if ( ! empty($this->image_sections) ){				
				...
				foreach ($this->image_sections as $k => $section) {
					...
					realestate_sync_API::add_additional_images_section($section['title'], $section['slug'], $current_values, '', true, false, $section['type']);
				}
			}
			...
			<div class="wpallimport-collapsed wpallimport-section wpallimport-addon '.$this->slug.' closed">
				<div class="wpallimport-content-section">
					<div class="wpallimport-collapsed-header">
						<h3>'.__($name,'realestate_sync_plugin').'</h3>	
					</div>
		...
		*/
		function import_images( $slug, $title, $type = 'images', $callback = NULL ){
			...
			if (is_array($slug)) {
                $section_slug = 'realestate_sync_' . md5(serialize($slug));
            } else {
                $section_slug = 'realestate_sync_' . $slug;
            }
			...
			/**
			*
			* filter to allow import images for free edition of RealEstate Sync
			*
			*/
		...
		*/		
		...
		function display_admin_notice() {
			...
			if ($this->notice_text) {
				...
				$notice_text = $this->name.' is integrated into RealEstate Sync Plugin.';
			}
