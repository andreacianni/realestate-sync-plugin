...
if ( ! class_exists( 'RealEstate_Sync_AddOn_Importer_Properties' ) ) {
    class RealEstate_Sync_AddOn_Importer_Properties extends RealEstate_Sync_AddOn_Importer {
        ...
        public function import_text_image_custom_details( $post_id, $data, $import_options, $article ) {
			...
			add_action( 'realestate_sync_saved_post', array( $this, 'save_in_slider' ), 1, 3 );
			add_action( 'realestate_sync_saved_post', array( $this, 'update_hidden_address' ), 1, 3 );

			if ( function_exists( 'estate_save_postdata' ) ) {
				add_action( 'realestate_sync_saved_post', 'estate_save_postdata', 1, 1 );
			}
        }
        ...
        public function import_property_location( $post_id, $data, $import_options, $article ) {
            $location_importer = new RealEstate_Sync_AddOn_Importer_Location( $this->add_on );
    ...
    if ( ! function_exists( 'property_images' ) ) {
        function property_images( $post_id, $attachment_id, $image_filepath, $import_options ) {
            // page_custom_image
            $wpresidence_addon = new RealEstate_Sync_AddOn_Helper();
    ...
    if ( ! function_exists( 'wpai_res_add_on_import_floorplan_images' ) ) {
        function wpai_res_add_on_import_floorplan_images( $post_id, $attachment_id, $image_filepath, $import_options ) {
            ...
            $save_floorplan_images_in_gallery = apply_filters( 'realestate_sync_is_save_floorplan_images_to_gallery', false, $post_id, $attachment_id, $image_filepath, $import_options );
